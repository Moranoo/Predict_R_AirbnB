...
Placez votre fichier de données CSV listings.csv dans le même dossier que le script.
Exécutez le script dans R ou RStudio avec la commande suivante :
R
Copier le code
...
Prédire le Prix : Entrez les caractéristiques du logement (accommodates, bathrooms, bedrooms) et choisissez la catégorie de quartier.
Cliquez sur "Prédire le prix" pour obtenir le prix estimé pour les deux modèles, y compris les frais supplémentaires (frais de ménage et taxes).
